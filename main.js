...
d3.json("./user.json").then(function(data){
    legendVals = d3.set(data.map( function(d) { return d.building } ) ).values();
    ...
    var color=d3.scaleOrdinal().domain(legendVals)
                                .range(["#1F77B4", "#FF7F0E", "#2CA02C","#8A3DA0","#22795F"
                                ]);

    var scaleX=d3.scaleBand().domain(data.map(d=>d.building))
                            .range([0,width])
                            .paddingInner(.4)
                            .paddingOuter(.4);
    ...
    var scaleY=d3.scaleLinear().domain([0,d3.max(data,(d)=>d.height)])
                                .range([height,0])
    ...
    g.append("g").attr("class","y-axis").call(y)
